...
export class AuthService {
  ...
  constructor(
    ...
    this.allowedMimeTypes = this.configService.get<string[]>("portfolio.allowedMimeTypes", [
      "image/jpeg",
      "image/png",
      "application/pdf",
    ])
  ...
  async checkUserTeamPermission(userId: string, teamId: string, permission: string): Promise<boolean> {
    return this.teamService.checkTeamPermission(teamId, userId, permission as any)
  }
  ...
  async sendVerificationEmail(email: string): Promise<void> {
    ...
    await this.emailTokenRepository.update({ userId: user.id, used: false }, { used: true })
  ...
  async getUsersWithFilters(page: number, limit: number, role?: UserRole, isSuspended?: boolean) {
    ...
    const [users, total] = await queryBuilder.skip(skip).take(limit).orderBy("user.createdAt", "DESC").getManyAndCount()

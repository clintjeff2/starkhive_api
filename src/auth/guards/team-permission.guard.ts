...
export const TeamPermission = (permission: string) => Reflect.metadata(TEAM_PERMISSION_KEY, permission)
...
export class TeamPermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const requiredPermission = this.reflector.getAllAndOverride<string>(TEAM_PERMISSION_KEY, [
      context.getHandler(),
      context.getClass(),
    ])

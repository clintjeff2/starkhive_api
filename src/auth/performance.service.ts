...
export class PerformanceService {
  ...
  async getFreelancerPerformance(
    ...
    const applicationSuccessRate =
      totalApplications > 0
        ? successfulApplications.length / totalApplications
        : 0;

    // Calculate earnings from approved applications
    const totalEarnings = successfulApplications.reduce(
      (sum, app) => sum + (app.job?.budget || 0),
      0,
    );
    ...
    successfulApplications.forEach((app) => {
      if (app.job?.createdAt) {
        ...
        earningsByMonth[month] =
          (earningsByMonth[month] || 0) + (app.job.budget || 0);
      }
    });
    ...
    const comparisonToAverage =
      avgSuccessRate > 0
        ? (applicationSuccessRate - avgSuccessRate) / avgSuccessRate
        : 0;

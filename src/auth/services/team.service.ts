...
import { type TeamMember, TeamRole, TeamMemberStatus } from "../entities/team-member.entity"
import { type TeamActivity, ActivityType } from "../entities/team-activity.entity"
...
export class TeamService {
  ...
  async createTeam(ownerId: string, createTeamDto: CreateTeamDto): Promise<Team> {
    ...
    // Log activity
    await this.logActivity(savedTeam.id, ownerId, ActivityType.TEAM_CREATED, "Team created", {
  ...
  async getTeamsByUser(userId: string, query: GetTeamsDto) {
    ...
    if (search) {
      queryBuilder.andWhere("team.name ILIKE :search", { search: `%${search}%` })
    }

    const [teams, total] = await queryBuilder.orderBy("team.createdAt", "DESC").skip(skip).take(limit).getManyAndCount()
  ...
  async getTeamById(teamId: string, userId: string): Promise<Team> {
    ...
    const hasAccess =
      team.ownerId === userId ||
      team.members.some((member) => member.userId === userId && member.status === TeamMemberStatus.ACTIVE)
  ...
  async updateTeam(teamId: string, userId: string, updateTeamDto: UpdateTeamDto): Promise<Team> {
    ...
    // Log activity
    await this.logActivity(teamId, userId, ActivityType.TEAM_SETTINGS_UPDATED, "Team settings updated")

    return this.getTeamById(teamId, userId)
  }

  async inviteTeamMember(teamId: string, inviterId: string, inviteDto: InviteTeamMemberDto): Promise<TeamMember> {
    ...
    if (!inviterMember.hasPermission("canInviteMembers")) {
      throw new ForbiddenException("Insufficient permissions to invite team members")
    }

    // Find user by email
    const user = await this.userRepository.findOne({ where: { email: inviteDto.email } })
    if (!user) {
    ...
    // Log activity
    await this.logActivity(teamId, inviterId, ActivityType.MEMBER_ADDED, "Team member invited", {
  ...
  ): Promise<{ successful: TeamMember[]; failed: { email: string; error: string }[] }> {
    const successful: TeamMember[] = []
    const failed: { email: string; error: string }[] = []

    for (const invitation of inviteDto.invitations) {
      try {
        const member = await this.inviteTeamMember(teamId, inviterId, invitation)
  ...
  async updateTeamMember(
    ...
    if (!updaterMember.hasPermission("canRemoveMembers") && team.ownerId !== updaterId) {
    ...
    if (updateDto.role && oldRole !== updateDto.role) {
      await this.logActivity(teamId, updaterId, ActivityType.MEMBER_ROLE_CHANGED, "Team member role updated", {
  ...
  async removeTeamMember(
    ...
    if (!removerMember.hasPermission("canRemoveMembers") && team.ownerId !== removerId) {
    ...
    // Log activity
    await this.logActivity(teamId, removerId, ActivityType.MEMBER_REMOVED, "Team member removed", {
      targetUserId: teamMember.userId,
      targetUserEmail: teamMember.user.email,
  ...
  async acceptTeamInvitation(teamId: string, userId: string): Promise<TeamMember> {
    const teamMember = await this.teamMemberRepository.findOne({
  ...
  async getTeamActivities(teamId: string, userId: string, query: GetTeamActivitiesDto) {
    ...
    if (filterUserId) {
      queryBuilder.andWhere("activity.userId = :filterUserId", { filterUserId })
    }

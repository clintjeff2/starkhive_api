...
export const recommendationConfig: RecommendationConfig = {
  popularity: {
    maxApplications: parseInt(process.env.RECOMMENDATION_MAX_APPLICATIONS || '100', 10),
    ...
    weights: {
      applicationScore: parseFloat(process.env.RECOMMENDATION_APPLICATION_WEIGHT || '0.4'),
  ...
  scoring: {
    weights: {
      ...
      experienceMatch: parseFloat(process.env.RECOMMENDATION_EXPERIENCE_WEIGHT || '0.20'),
      locationMatch: parseFloat(process.env.RECOMMENDATION_LOCATION_WEIGHT || '0.15'),
      budgetMatch: parseFloat(process.env.RECOMMENDATION_BUDGET_WEIGHT || '0.15'),
      userBehavior: parseFloat(process.env.RECOMMENDATION_BEHAVIOR_WEIGHT || '0.15'),
      jobPopularity: parseFloat(process.env.RECOMMENDATION_POPULARITY_WEIGHT || '0.10'),
    },
  },
  limits: {
    defaultRecommendationLimit: parseInt(process.env.RECOMMENDATION_DEFAULT_LIMIT || '20', 10),
    maxRecommendationLimit: parseInt(process.env.RECOMMENDATION_MAX_LIMIT || '100', 10),
    defaultBudgetMax: parseInt(process.env.RECOMMENDATION_DEFAULT_BUDGET_MAX || '10000', 10),
    maxBudgetLimit: parseInt(process.env.RECOMMENDATION_MAX_BUDGET_LIMIT || '1000000', 10),
  },
  behavior: {
    baseBehaviorScore: parseFloat(process.env.RECOMMENDATION_BASE_BEHAVIOR_SCORE || '0.5'),
    applicationBonus: parseFloat(process.env.RECOMMENDATION_APPLICATION_BONUS || '0.3'),
...
export function validateRecommendationConfig(config: RecommendationConfig): void {
  ...
  if (Math.abs(popularityWeightSum - 1.0) > 0.01) {
    errors.push(`Popularity weights must sum to 1.0, got ${popularityWeightSum}`);
  }

  // Validate scoring weights sum to 1
  const scoringWeightSum = Object.values(config.scoring.weights).reduce((sum, weight) => sum + weight, 0);
  if (Math.abs(scoringWeightSum - 1.0) > 0.01) {
    errors.push(`Scoring weights must sum to 1.0, got ${scoringWeightSum}`);
  }

  // Validate limits
  if (config.limits.defaultRecommendationLimit > config.limits.maxRecommendationLimit) {
    errors.push('Default recommendation limit cannot be greater than max limit');
  }
  ...
  if (config.behavior.baseBehaviorScore < 0 || config.behavior.baseBehaviorScore > 1) {
    errors.push('Base behavior score must be between 0 and 1');
  }

  if (config.behavior.applicationBonus < 0 || config.behavior.applicationBonus > 1) {
  ...
  if (errors.length > 0) {
    throw new Error(`Invalid recommendation configuration: ${errors.join(', ')}`);
  }
}

...
export class AntiSpamService {
  private readonly blacklist = ['easy money', 'click here', 'work from home', 'free', 'guaranteed'];
  ...
  async analyzeJobPost(job: Job): Promise<boolean> {
    ...
    if (matched) {
      await this.flagRepo.save({ jobId: job.id, reason: `Matched keyword: ${matched}` });
      return true;
    }
    ...
    if (filtered.length > 3) {
      await this.flagRepo.save({ jobId: job.id, reason: 'Flood control: too many posts in short time' });
      return true;
    }
